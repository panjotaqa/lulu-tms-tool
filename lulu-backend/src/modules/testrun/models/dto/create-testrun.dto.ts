...
export class EndDateAfterStartDateConstraint implements ValidatorConstraintInterface {
  validate(endDate: string, _args: ValidationArguments): boolean {
    const dto = _args.object as CreateTestRunDto;
...
export class CreateTestRunDto {
  ...
  @IsDateString(
    {},
    { message: 'Data de início deve ser uma data válida (formato ISO 8601)' },
  )
  startDate: string;
  ...
  @IsDateString(
    {},
    { message: 'Data de término deve ser uma data válida (formato ISO 8601)' },
  )
  ...
  @ApiProperty({
    description:
      'Array de IDs dos casos de teste a serem incluídos na execução',
    example: [
