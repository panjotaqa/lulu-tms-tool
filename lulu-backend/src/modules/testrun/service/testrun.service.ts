...
import { ITestCaseSnapshot } from '../models/interfaces/testcase-snapshot.interface';
...
export class TestRunService implements ITestRunService {
  constructor(
    @Inject(TESTRUN_REPOSITORY)
    private readonly testRunRepository: TestRunRepository,
    @Inject(TESTRUNCASE_REPOSITORY)
    private readonly testRunCaseRepository: TestRunCaseRepository,
  ...
  async create(
    ...
    if (!isUserInProject) {
      throw new BadRequestException('Usuário não tem acesso a este projeto');
    }
    ...
    if (testCaseIds.length > 0) {
      ...
      const allInSameProject = testCases.every(
        (testCase) =>
          testCase.testSuite.projectId === createTestRunDto.projectId,
      );
    ...
    try {
      ...
      for (const testCase of testCases) {
        ...
        const testCaseSnapshot: ITestCaseSnapshot = {
          id: testCase.id,
        ...
        const testRunCase = queryRunner.manager.create(TestRunCase, {
          ...
          testCaseSnapshot: testCaseSnapshot as unknown as object,
  ...
  async updateTestCaseStatus(
    ...
    this.debugLogger.debug(
      'TestRunService',
      'Atualizando status do caso de teste',
      {
        testRunId,
        testRunCaseId,
    ...
    const updatedTestRunCase =
      await this.testRunCaseRepository.save(testRunCase);
  ...
  async updateTestCaseEvidence(
    ...
    this.debugLogger.debug(
      'TestRunService',
      'Atualizando evidências do caso de teste',
      {
        testRunId,
        testRunCaseId,
      },
