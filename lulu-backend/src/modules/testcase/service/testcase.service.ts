...
export class TestCaseService implements ITestCaseService {
  ...
  async create(
    ...
    try {
      ...
      const nextNumber = await this.testCaseRepository.getNextTestCaseSequence(
        folder.projectId,
        queryRunner.manager,
      );
  ...
  async update(
    ...
    try {
      ...
      const updatedTestCase = await queryRunner.manager.save(
        TestCase,
        testCase,
      );
  ...
  async createBulk(
    ...
    const initialCount =
      (await this.testCaseRepository.getNextTestCaseSequence(
        folder.projectId,
        queryRunner.manager,
      )) - 1;
    ...
    try {
      ...
      for (const title of validTitles) {
        try {
          ...
          const savedTestCase = await queryRunner.manager.save(
            TestCase,
            testCase,
          );
      ...
      for (const id of savedTestCaseIds) {
        try {
          ...
          this.debugLogger.debug(
            'TestCaseService',
            'Erro ao buscar caso de teste criado',
            {
              id,
              error: error instanceof Error ? error.message : String(error),
            },
      ...
      this.debugLogger.debug(
        'TestCaseService',
        'Casos de teste criados em massa',
        {
      ...
      this.debugLogger.debug(
        'TestCaseService',
        'Erro ao criar casos de teste em massa',
        {
          error: error instanceof Error ? error.message : String(error),
        },
      );
  ...
  async moveTestCases(
    ...
    if (testCaseIds.length === 0) {
      throw new BadRequestException(
        'É necessário selecionar pelo menos um caso de teste',
      );
    }
  ...
  async findByFolder(
    ...
    const { testCases, total } =
      await this.testCaseRepository.findByFolderPaginated(folderId, queryDto);
  ...
  async findByProjectForSelection(
    ...
    return this.testCaseRepository.findForSelection(projectId);
  }
